...
export async function createSip({ fundId, amount, frequency = 'monthly', durationMonths, debitDay, stepUp, consentTextVersion, consentedAt }: any) {
  if (useHttpApi()) {
...
export async function createLumpsum({ fundId, amount }: any) {
  if (useHttpApi()) {
...
export async function listOrders({ filter = 'all' }: any = {}) {
  if (useHttpApi()) {
...
export async function requestSipControl({ orderId, requestType, requestedValue, effectiveDate, reason }: any) {
  if (useHttpApi()) {
