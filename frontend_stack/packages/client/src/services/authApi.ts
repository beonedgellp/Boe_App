import { fixtureUser } from '../data/fixtureUser';
import {
  apiRequest,
  clearSessionTokens,
  clone,
...
export async function login(credentials: any = {}, { scope = 'client' }: any = {}) {
  if (useHttpApi()) {
...
export async function signup(details: any = {}, { scope = 'client' }: any = {}) {
  if (useHttpApi()) {
...
export async function logout({ scope = 'client' }: any = {}) {
  if (useHttpApi()) {
    try {
...
export async function currentUser({ scope = 'client' }: any = {}) {
  if (useHttpApi()) {
    let current;
    try {
