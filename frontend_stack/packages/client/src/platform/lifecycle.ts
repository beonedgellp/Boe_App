...
function addDomListener(target, event, handler, options?: any) {
...
export const platformLifecycle = {
  ...
  onPause(callback) {
    const cleanups = [
      addAppStateListener(({ isActive }: any) => {
  ...
  onVisibilityChange(callback) {
    const cleanups = [
      addAppStateListener(({ isActive }: any) => callback(isActive ? 'visible' : 'hidden')),
    ];
