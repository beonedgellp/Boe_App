...
function formatNumberMoney(n, { decimals = 0, sign = false }: any = {}) {
...
export function fmtMoney(n, { decimals = 0, sign = false, source, asOf, currency = 'INR' }: any = {}) {
...
export function formatMoney(amount, { source, asOf, currency = 'INR', decimals = 0, sign = false }: any = {}) {
  if (source === undefined) {
...
export function fmtPct(n, { sign = true, decimals = 1 }: any = {}) {
  if (n === null || n === undefined || Number.isNaN(n)) return '—';
  const neg = n < 0;
...
export function fmtDate(iso, { withTime = false }: any = {}) {
  if (!iso) return '—';
