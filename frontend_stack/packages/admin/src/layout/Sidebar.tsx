...
import { NAV_DOMAINS } from '../navigation/nav';
import { initials, displayRole } from '../helpers/formatters';
...
function AdminUserChip({ user, collapsed = false, mobile = false, className = '' }: any) {
...
function SidebarGroup({ domain, counts, isOpen, onToggle, isMobile, isCollapsed }: any) {
...
function SidebarItem({ item, counts }: any) {
  const count = item.badge ? Number(counts[item.badge]) : 0;
  return (
    <NavLink
      to={item.path}
      className={({ isActive }: any) => `ash-nav-item ${isActive ? 'is-active' : ''}`}
    >
...
export default function Sidebar({ user, counts = {}, collapsed = false }: any) {
