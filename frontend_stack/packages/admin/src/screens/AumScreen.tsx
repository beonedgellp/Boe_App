...
import { fmtInt, clone, initials } from '../helpers/formatters';
import AumAllocationsTab from './AumAllocationsTab';
import AumCapitalTab from './AumCapitalTab';
import AumRedemptionsTab from './AumRedemptionsTab';
import AumDisplayFields from './AumDisplayFields';

function AumScreen({ funds = [], auditLogs = [], onCreate, onUpdate, onDelete, onUserDetail }: any) {
  ...
  const selectedFundAuditLogs = selectedFund
    ? (auditLogs || []).filter(log =>
      ...
      ).sort((a, b) => new Date(b.timestamp || b.createdAt || 0).getTime() - new Date(a.timestamp || a.createdAt || 0).getTime()).slice(0, 10)
    : [];
  return (
    <>
      {editorOpen ? (
            ...
            <div className="adm-fund-editor-main">
              ...
              <section id="basic" className="adm-fund-editor-section">
                ...
                {(() => {
                  ...
                  const ageDays = hasAge ? Math.floor((now.getTime() - launch.getTime()) / (1000 * 60 * 60 * 24)) : 0;
