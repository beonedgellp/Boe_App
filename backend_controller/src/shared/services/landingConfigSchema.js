...
const text = (options = {}) => ({ kind: 'string', max: TEXT_MAX, ...options });
const longText = (options = {}) => ({ kind: 'string', max: LONG_TEXT_MAX, ...options });
const href = (options = {}) => ({ kind: 'string', max: TEXT_MAX, pattern: HREF_PATTERN, patternHint: 'must start with /, #, or https://', ...options });
const link = (options = {}) => ({
  kind: 'object',
  fields: { label: text({ required: true }), href: href({ required: true }) },
  ...options,
});
const list = (item, maxItems, options = {}) => ({ kind: 'array', item, maxItems, ...options });
...
export function validateLandingConfig(input) {
